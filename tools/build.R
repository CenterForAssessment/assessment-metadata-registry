...
#
# Routed by `schema_version`:
#   amr.assessment_system.v1      -> identity, scales, levels, cutscores
#   amr.accountability_system.v1  -> achievement TARGETS (state policy), cross-linked
#
# Emits, under `build/` (git-ignored, regenerated every run):
#   manifest.json     provenance: git SHA, built_at, schema versions, counts, per-file sha256
#   index.json        flat: one row per jurisdiction x system x year x content area
#   changelog.json    year-over-year diffs (assessment fields + targets)
#   targets.json      flat accountability targets (cross-linked)
#   dist/<JUR>.json + dist/<JUR>/<system>.json  bundles
#   tables/vendor_by_year.json + tables/vertical_scale.json  cross-cutting views
#   registry.sqlite   self-contained SQLite projection (amr.registry.v1)
#
...
# ---- derived views ----------------------------------------------------------------
build_index <- function(records) {
  rows <- list()
  for (rec in records) {
    if (!is_assessment(rec)) next
  ...
  order_rows(rows, c("jurisdiction_id", "assessment_system_id", "year", "content_area"))
}

build_targets <- function(records) {
  rows <- list()
  for (rec in records) {
    if (!is_accountability(rec)) next
    ...
    for (t in rec$targets %||% list()) {
      rows[[length(rows) + 1L]] <- list(
        ...
        semantics = t$semantics, basis = t$basis,
        comparison = t$comparison, label = t$label,
        has_per_grade_scale_score = length(t$per_grade_scale_score %||% list()) > 0
      )
    }
  }
  order_rows(rows, c("jurisdiction_id", "accountability_system_id",
                     "assessment_system_id", "content_area", "year"))
}

build_vendor_by_year <- function(records) {
  rows <- list()
  for (r in records) {
    if (!is_assessment(r)) next
    rows[[length(rows) + 1L]] <- list(
      jurisdiction_id = r$jurisdiction$id,
      assessment_system_id = r$assessment_system$id,
      year = r$administration$year, vendor = r$administration$vendor)
  }
  order_rows(rows, c("jurisdiction_id", "assessment_system_id", "year"))
}

build_vertical_scale <- function(records) {
  rows <- list()
  for (r in records) {
    if (!is_assessment(r)) next
    for (ca in r$content_areas %||% list()) {
      rows[[length(rows) + 1L]] <- list(
        ...
        vertical_scale = isTRUE(ca$vertical_scale %||% FALSE),
        scale_name = ca$scale_name)
    }
  }
  order_rows(rows, c("jurisdiction_id", "assessment_system_id", "content_area", "year"))
}

build_changelog <- function(records) {
  events <- list()
  ...
  for (r in records) if (is_assessment(r)) {
    k <- paste(r$jurisdiction$id, r$assessment_system$id, sep = SEP)
  ...
  for (k in sort(names(a_series))) {
    ...
    for (i in seq_len(length(recs) - 1L)) {
      ...
      base <- list(record_type = "assessment", jurisdiction_id = jid,
      ...
      for (ca in sort(unique(c(names(pv), names(cv))))) {
        if (!identical(pv[[ca]], cv[[ca]])) {
          add(c(base, list(content_area = ca, field = "vertical_scale",
                           from = pv[[ca]], to = cv[[ca]])))
        }
      }
      ...
      for (ca in sort(unique(c(names(pl), names(cl))))) {
        if (!identical((pl[[ca]] %||% list())$labels, (cl[[ca]] %||% list())$labels)) {
          add(c(base, list(content_area = ca, field = "achievement_levels",
                           from = (pl[[ca]] %||% list())$labels,
                           to = (cl[[ca]] %||% list())$labels)))
        }
      }
      pc <- prev$cutscores %||% list(); cc2 <- cur$cutscores %||% list()
      for (ca in sort(unique(c(names(pc), names(cc2))))) {
        if (!identical(pc[[ca]], cc2[[ca]])) {
          add(c(base, list(content_area = ca, field = "cutscores",
                           from = pc[[ca]], to = cc2[[ca]])))
        }
      }
    }
  }

  # target diffs per (jurisdiction, accountability_system)
  target_map <- function(rec) {
    m <- list()
    for (t in rec$targets %||% list()) {
      m[[paste(t$assessment_system_id, t$content_area, sep = SEP)]] <- t
    }
    m
  }
  b_series <- list()
  for (r in records) if (is_accountability(r)) {
    k <- paste(r$jurisdiction$id, r$accountability_system$id, sep = SEP)
    ...
    for (i in seq_len(length(recs) - 1L)) {
      ...
      for (kk in sort(unique(c(names(pm), names(cm))))) {
        ...
        if (!identical(p[fields], c_[fields])) {
          parts <- strsplit(kk, SEP, fixed = TRUE)[[1]]
...
# ---- sqlite -----------------------------------------------------------------------
build_sqlite <- function(records, db_path, ddl_path, prov) {
  ...
  for (stmt in strsplit(paste(lines, collapse = "\n"), ";", fixed = TRUE)[[1]]) {
    s <- trimws(stmt); if (nzchar(s)) dbExecute(con, s)
  }
  for (kv in list(list("git_sha", prov$sha %||% NA), list("built_at", prov$built_at),
                  list("schema_version", "amr.registry.v1"))) {
    dbExecute(con, "INSERT INTO registry_meta(key, value) VALUES (?, ?)", params = kv)
  }
  ...
  for (r in records) {
    ...
    if (!(jur$id %in% seen_jur)) {
      dbExecute(con, "INSERT OR REPLACE INTO jurisdiction VALUES (?,?,?,?,?)",
    ...
    if (is_assessment(r)) {
      ...
      if (!(sys_$id %in% seen_sys)) {
        dbExecute(con, "INSERT OR REPLACE INTO assessment_system VALUES (?,?,?,?)",
                  params = list(sys_$id, sys_$name, sys_$family, sys_$assessment_type))
        seen_sys <- c(seen_sys, sys_$id)
      }
      prv <- r$provenance %||% list()
      dbExecute(con, "INSERT OR REPLACE INTO administration VALUES (?,?,?,?,?,?,?,?,?,?)",
      ...
      dbExecute(con, "INSERT OR REPLACE INTO assessment_program VALUES (?,?,?,?,?,?,?,?)",
      ...
      if (!is.null(comp)) {
        dbExecute(con, "INSERT OR REPLACE INTO comparability VALUES (?,?,?,?,?,?,?,?)",
      ...
      for (ca in r$content_areas %||% list()) {
        dbExecute(con, "INSERT OR REPLACE INTO vertical_scale VALUES (?,?,?,?,?,?,?)",
      ...
      for (ca in names(r$achievement_levels %||% list())) {
        ...
        for (i in seq_along(labels)) {
          ...
          dbExecute(con, "INSERT OR REPLACE INTO achievement_level VALUES (?,?,?,?,?,?,?)",
                    params = list(jur$id, sys_$id, ca, as.character(adm$year), i - 1L, labels[[i]],
                                  if (is.null(pf)) NA_integer_ else if (as_bool(pf)) 1L else 0L))
        }
      }
...
# ---- io ---------------------------------------------------------------------------
clean_record <- function(rec) { rec[["_source_path"]] <- NULL; rec }
...
# ---- main -------------------------------------------------------------------------
main <- function(argv) {
  opts <- parse_args(argv, list(metadata = "metadata", out = "build",
                                ddl = "schemas/sql/amr-registry.v1.sql"))
  metadata_root <- opts$metadata; out <- opts$out
  repo <- if (basename(metadata_root) == "metadata") dirname(metadata_root) else "."

  records <- load_records(metadata_root)
  if (!length(records)) { message(sprintf("No sidecars under %s", metadata_root)); return(1L) }
  ...
  write_json(file.path(out, "index.json"), list(`_registry` = stamp, records = build_index(records)))
  write_json(file.path(out, "targets.json"), list(`_registry` = stamp, rows = build_targets(records)))
  write_json(file.path(out, "changelog.json"), list(`_registry` = stamp, events = build_changelog(records)))
  write_json(file.path(out, "tables", "vendor_by_year.json"),
             list(`_registry` = stamp, rows = build_vendor_by_year(records)))
  write_json(file.path(out, "tables", "vertical_scale.json"),
             list(`_registry` = stamp, rows = build_vertical_scale(records)))

  by_jur <- list(); by_sys <- list()
  for (r in records) {
    jid <- r$jurisdiction$id
    by_jur[[jid]] <- c(by_jur[[jid]], list(clean_record(r)))
    sk <- paste(jid, system_id(r), sep = SEP)
    by_sys[[sk]] <- c(by_sys[[sk]], list(clean_record(r)))
  }
  for (jid in names(by_jur)) {
    write_json(file.path(out, "dist", paste0(jid, ".json")),
               list(`_registry` = stamp, jurisdiction_id = jid, records = by_jur[[jid]]))
  }
  for (sk in names(by_sys)) {
    parts <- strsplit(sk, SEP, fixed = TRUE)[[1]]
    write_json(file.path(out, "dist", parts[1], paste0(parts[2], ".json")),
               list(`_registry` = stamp, jurisdiction_id = parts[1],
                    system_id = parts[2], records = by_sys[[sk]]))
  }

  build_sqlite(records, file.path(out, "registry.sqlite"), opts$ddl, prov)

  n_assess <- sum(vapply(records, is_assessment, logical(1)))
  n_acct <- sum(vapply(records, is_accountability, logical(1)))
  all_files <- sort(list.files(out, recursive = TRUE, full.names = TRUE), method = "radix")
  files <- list()
  for (f in all_files) {
    if (basename(f) == "manifest.json") next
    rel <- sub(paste0("^", out, "/"), "", f)
    files[[rel]] <- digest::digest(file = f, algo = "sha256")
  }
  write_json(file.path(out, "manifest.json"), list(
    `_registry` = stamp, n_records = length(records),
